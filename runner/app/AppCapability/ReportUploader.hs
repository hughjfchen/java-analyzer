...
import Path (toFilePath, fromSomeFile)
import Data.Password.Types (unsafeShowPassword)

import qualified Text.URI as URI

data LoginReq = LoginReq { loginReqEmail :: ! Text
                         , loginReqPassword :: ! Text
                         } deriving stock (Eq, Show, Generic)

instance ToJSON LoginReq where
  toJSON LoginReq{..} = Aeson.object [ "email" Aeson..= loginReqEmail
                                     , "password" Aeson..= loginReqPassword
                                     ]

data MeInfo = MeInfo { meId :: ! Int
                     , meName :: ! Text
                     , meEmail :: ! Text
                     , meRole :: ! Text
                     } deriving stock (Eq, Show, Generic)
...
--     pure $ MeInfo{..}
instance FromJSON MeInfo where
  parseJSON (Aeson.Object o) = MeInfo <$> (o .: "id")
                         <*> (o .: "name")
                         <*> (o .: "email")
                         <*> (o .: "role")
  parseJSON _ = mzero

data LoginInfo = LoginInfo { loginInfoMe :: MeInfo
                           , loginInfoToken :: ! Text
                           } deriving stock (Eq, Show, Generic)
instance FromJSON LoginInfo where
  parseJSON (Aeson.Object o) = LoginInfo <$> (o .: "me")
                         <*> (o .: "token")
  parseJSON _ = mzero
...
instance ReportUploaderM AppM' where
  uploadReport jobId reportPath = do
    ...
    loginReq <- pure $ LoginReq { loginReqEmail = unsafeShowPassword $ curlCmdLineLoginUser' curlOptions
                                , loginReqPassword = unsafeShowPassword $ curlCmdLineLoginPIN' curlOptions
                                }
    (loginOut, _) <- readProcess_ $ proc (fromSomeFile $ cmdCurlPath' cmdPaths) ["--header", "Content-Type: application/json"
                                                                , "--header", "Accept: application/json"
                                                                , "--fail"
                                                                , "--data", decodeUtf8 $ Aeson.encode loginReq
    ...
    runProcess_ $ proc (fromSomeFile $ cmdCurlPath' cmdPaths) ["--header", "Authorization: Bearer " <> toString token
                                              , "--fail"
                                              , "--form", "job_id=" <> show jobId
                                              ...
                                              , "--form", "report=@" <> toFilePath reportPath
